...
{ 	struct sigaction sa;
    ...
    //insert into MySQL
	//if(t1>0){
        sprintf(query, "insert into ScalabilityLog (Timestamp,Server,ElapsedTime,NumOfHosts,ChunkSize,Size,MFLOPS) values (\"%s\",\"%s\",%f,%d,%d,%d,%f)",
			  timeStamp,host,t1/1000000,P,G,N+1,(float) N*N/t1*N);
	//}else{
	//	sprintf(query, "insert into ScalabilityLog (Timestamp,Host,ElapsedTime,NumOfCores(P),ChunkSize(f),Size(n),MFLOPS) values (\"%s\",\"%s\",%f,%d,%d,%d,\"Not measured.\")",
	//		  timeStamp,host,t1/1000000,P,G,N+1);
	//}
